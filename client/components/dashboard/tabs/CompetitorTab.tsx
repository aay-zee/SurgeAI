...
export function CompetitorTab({ competitors, onExtract }: CompetitorTabProps) {
  ...
  // Normalise weaknesses/strengths to string[] regardless of format
  const toStringArray = (items: any): string[] => {
    if (!items) return [];
  ...
  const sortedCompetitors = React.useMemo(() => {
    ...
    if (sortBy === "rating") {
      return entries.sort((a, b) => (b[1].rating ?? 0) - (a[1].rating ?? 0));
    } else if (sortBy === "reviews") {
      return entries.sort((a, b) => (b[1].review_count ?? 0) - (a[1].review_count ?? 0));
    } else {
          ...
          <motion.div variants={itemVariants}>
            <Card>
              ...
              <CardContent className="space-y-4">
                {(() => {
                  // Count weaknesses across all competitors (handles both string[] and Record formats)
                  const allWeaknesses: Record<string, number> = {};
                  Object.values(competitors).forEach((app) => {
                    toStringArray(app.weaknesses).forEach((item) => {
                      allWeaknesses[item] = (allWeaknesses[item] || 0) + 1;
                    });
                  });
                  ...
                  // Count strengths across all competitors
                  const allStrengths: Record<string, number> = {};
                  Object.values(competitors).forEach((app) => {
                    toStringArray(app.strengths).forEach((item) => {
                      allStrengths[item] = (allStrengths[item] || 0) + 1;
                    });
                  });
