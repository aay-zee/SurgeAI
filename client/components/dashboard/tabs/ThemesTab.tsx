...
export function ThemesTab({ themes, onExtract }: ThemesTabProps) {
  ...
  const sortedThemes = React.useMemo(() => {
    ...
    if (sortBy === "frequency") {
      ...
      return entries.sort((a, b) => (a[1].sentiment ?? 0) - (b[1].sentiment ?? 0)); // Most negative first
    } else {
            ...
            {sortedThemes.map(([themeName, themeData]) => (
              <motion.div key={themeName} variants={itemVariants}>
                <ThemeCard
                  ...
                  sentiment={themeData.sentiment ?? 0}
              ...
              <CardContent className="space-y-3 text-sm">
                {(() => {
                  ...
                  const mostNegative = Object.entries(themes).sort(
                    (a, b) => (a[1].sentiment ?? 0) - (b[1].sentiment ?? 0)
                  )[0];
                      ...
                      <div className="flex gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">
                            ...
                            {(mostNegative[1].sentiment ?? 0).toFixed(2)}
                          </p>
                        </div>
                      </div>
