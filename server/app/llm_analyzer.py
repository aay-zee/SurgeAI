...
def batch_sentiment_analysis(
    ...
    """
    # Get all relevant posts — upsert will overwrite any prior per-post analysis
    # with higher-quality batched results that include reasoning
    posts = (
...
Score each dimension 1-10 with a specific reason based on the data above."""
    ...
    # Persist to ValidationScore table
...
def analyze_competitors_llm(
    ...
    # ── Step 1: LLM identifies real competitors ──────────────────────────────
    id_system = (
    ...
    try:
        ...
        if not isinstance(competitor_names, list) or not competitor_names:
            print("[Competitors] LLM returned no competitor names")
            return {}
    ...
    # ── Step 2: Fetch reviews from Google Play for each competitor ────────────
    try:
    ...
    for app_name in competitor_names:
        try:
            # Search top 3 candidates — try each until we find one with reviews
            raw = search(app_name, n_hits=3, lang="en", country="us")
            candidates = raw[0] if isinstance(raw, tuple) else raw
            if not candidates:
                print(f"[Competitors] No Play Store result for: {app_name}")
                continue
            ...
            if not chosen_app or not fetched_reviews:
                print(f"[Competitors] No reviews found for any candidate of: {app_name}")
                continue
            ...
            for rev in fetched_reviews[:30]:
                ...
                # Collect representative quotes for LLM analysis
                short = content[:180]
            ...
            print(f"[Competitors] {actual_name}: {len(review_texts)} reviews, "
                  f"{len(positive_quotes)} positive quotes, {len(negative_quotes)} negative quotes")
    ...
    # ── Step 3: LLM analyzes reviews for strengths / weaknesses / gaps ───────
    reviews_text = ""
    ...
    analysis_user = (
        ...
        "For each app include: strengths (3-5 items backed by review quotes), "
        "weaknesses (3-5 specific complaints from reviews), gaps (2-4 market opportunities), "
        "rating (number), user_sentiment (1 sentence), top_positive_quote, top_negative_quote. "
    ...
    try:
        ...
        # Fallback: LLM sometimes returns one JSON block per app instead of one combined object.
        # Find all JSON objects in the response and merge them using app names as keys.
        if not isinstance(result, dict) or not result:
            import re as _re
            merged = {}
            blocks = _re.findall(r"\{[\s\S]*?\}(?=\s*(?:\*\*|$|\n\n))", response)
            for i, block in enumerate(blocks):
                try:
                    parsed = json.loads(block)
                    if isinstance(parsed, dict) and any(k in parsed for k in
                            ["strengths", "weaknesses", "gaps", "rating"]):
                        key = app_names[i] if i < len(app_names) else f"competitor_{i+1}"
                        merged[key] = parsed
                except Exception:
                    continue
            if merged:
                print(f"[Competitors] Used fallback multi-block parser: {list(merged.keys())}")
                return merged
            print(f"[Competitors] JSON parse failed. Raw response (first 300 chars): {response[:300]}")
            return {}

        print(f"[Competitors] Analysis complete for: {list(result.keys())}")
        return result
    except Exception as e:
